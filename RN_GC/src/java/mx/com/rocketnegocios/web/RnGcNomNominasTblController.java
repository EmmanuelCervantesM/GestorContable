...
public class RnGcNomNominasTblController implements Serializable {
    ...
    // Variables para duplicar nomina
    ...
    public Date getFechaInicioDuplicado() {
        return fechaInicioDuplicado;
    }

    public void setFechaInicioDuplicado(Date fechaInicioDuplicado) {
        this.fechaInicioDuplicado = fechaInicioDuplicado;
    }

    public Date getFechaFinDuplicado() {
        return fechaFinDuplicado;
    }

    public void setFechaFinDuplicado(Date fechaFinDuplicado) {
        this.fechaFinDuplicado = fechaFinDuplicado;
    }
...
// Hacemos final para usar en el lambda
    ...
    // Función para duplicar nómina
    public void duplicarNomina(RnGcNomNominasTbl nominaId) {
        System.out.println("📝 Nuevo nombre de la nómina: " + nombreDuplicado);
        System.out.println("📅 Fecha de inicio: " + fechaInicioDuplicado);
        System.out.println("📅 Fecha de fin: " + fechaFinDuplicado);
        try {

            // Validar fechas
            if (fechaInicioDuplicado == null || fechaFinDuplicado == null) {
                JsfUtil.addErrorMessage("⚠️ Debes ingresar ambas fechas.");
                return;
            }

            if (!fechaFinDuplicado.after(fechaInicioDuplicado)) {
                JsfUtil.addErrorMessage("⚠️ La fecha de fin debe ser posterior a la fecha de inicio.");
                return;
            }

            System.out.println("📌 Iniciando duplicación de nómina con ID original: " + nominaId.getId());

            // Obtener nómina original
            RnGcNomNominasTbl nominaOriginal = nominaFacade.obtenerNominaPorIdUnico(nominaId.getId());
            if (nominaOriginal == null) {
                System.out.println("❌ Nómina no encontrada con ID: " + nominaId);
                return;
            }
            ...
            //nuevaNomina.setNombreNomina(nominaOriginal.getNombreNomina() + " - Copia");
            nuevaNomina.setNombreNomina(nombreDuplicado);
            nuevaNomina.setEsConfidencial(nominaOriginal.getEsConfidencial());
            nuevaNomina.setTipoNominaId(nominaOriginal.getTipoNominaId());
            nuevaNomina.setPeriodicidadPagoId(nominaOriginal.getPeriodicidadPagoId());
            nuevaNomina.setOrigenRecursoId(nominaOriginal.getOrigenRecursoId());
            nuevaNomina.setPatronId(usuarioFirmado.obtenerIdUsuario());

            // Agrega estos campos obligatorios
            nuevaNomina.setCreadoPor(usuarioFirmado.obtenerIdUsuario());
            nuevaNomina.setFechaCreacion(new Date());
            nuevaNomina.setUltimaActualizacionPor(usuarioFirmado.obtenerIdUsuario());
            nuevaNomina.setUltimaFechaActualizacion(new Date());

            // Persistir la nueva nómina
            nominaFacade.create(nuevaNomina);
            //nuevaNomina = nominaFacade.refreshFromDB(nuevaNomina);

            System.out.println("🆕 Nueva nómina creada: " + nuevaNomina.getNombreNomina() + " | ID: " + nuevaNomina.getId());
            ...
            nuevoPeriodo.setFechaCreacion(new Date());
            nuevoPeriodo.setUltimaActualizacionPor(usuarioFirmado.obtenerIdUsuario());
            nuevoPeriodo.setUltimaFechaActualizacion(new Date());

            //nuevoPeriodo.setFechaInicio(periodoOriginal.getFechaInicio());
            //nuevoPeriodo.setFechaFin(periodoOriginal.getFechaFin());
            nuevoPeriodo.setFechaInicio(fechaInicioDuplicado);
            nuevoPeriodo.setFechaFin(fechaFinDuplicado);

            String nombrePeriodo = "Periodo " + new SimpleDateFormat("dd/MM/yyyy").format(fechaInicioDuplicado) + " - Copia";
            nuevoPeriodo.setNombrePeriodo(nombrePeriodo);

            //nuevoPeriodo.setAnioPeriodo(periodoOriginal.getAnioPeriodo());
            //nuevoPeriodo.setNumMesPeriodo(periodoOriginal.getNumMesPeriodo());
            Calendar cal = Calendar.getInstance();
            cal.setTime(fechaInicioDuplicado);
            nuevoPeriodo.setAnioPeriodo(String.valueOf(cal.get(Calendar.YEAR)));
            nuevoPeriodo.setNumMesPeriodo(cal.get(Calendar.MONTH) + 1); // Enero = 0

            nuevoPeriodo.setNominaId(nuevaNomina);
            nuevoPeriodo.setEstatus(periodoOriginal.getEstatus());
            //nuevoPeriodo = periodoNominaFacade.refreshFromDB(nuevoPeriodo);
            periodoNominaFacade.create(nuevoPeriodo);
            ...
            nuevaSolicitud.setNominaId(nuevaNomina.getId());
            nuevaSolicitud.setPeriodoNominaId(nuevoPeriodo);
            nuevaSolicitud.setEstatus("A");
            nuevaSolicitud.setPatronId(usuarioFirmado.obtenerIdUsuario());

            // ⚠️ Campos obligatorios (evita ConstraintViolation)
            nuevaSolicitud.setFechaCreacion(new Date());
            nuevaSolicitud.setUltimaFechaActualizacion(new Date());
            nuevaSolicitud.setCreadoPor(usuarioFirmado.obtenerIdUsuario());
            nuevaSolicitud.setUltimaActualizacionPor(usuarioFirmado.obtenerIdUsuario());

            //nuevsaSolicitud = solicitudFacade.refreshFromDB(nuevaSolicitud);
            solicitudFacade.create(nuevaSolicitud);
            ...
            System.out.println("👷‍♂️ Total trabajadores a duplicar: " + trabajadoresOriginales.size());

            int contador = 1;
            for (RnGcNomSolicitudTrabajadorTbl original : trabajadoresOriginales) {
                RnGcNomSolicitudTrabajadorTbl nuevoTrabajador = new RnGcNomSolicitudTrabajadorTbl();
                nuevoTrabajador.setTrabajadorId(original.getTrabajadorId());
                nuevoTrabajador.setSolicitudId(nuevaSolicitud);
                nuevoTrabajador.setDiasPagados(diasPagados.intValue());
                nuevoTrabajador.setFechaPago(original.getFechaPago());
                nuevoTrabajador.setSdi(original.getSdi());
                nuevoTrabajador.setEstatus("A");
                nuevoTrabajador.setTotalDeducciones(original.getTotalDeducciones());
                nuevoTrabajador.setTotalPercepciones(original.getTotalPercepciones());
                nuevoTrabajador.setImporteNeto(original.getImporteNeto());

                // ⚠️ Agrega los campos requeridos
                nuevoTrabajador.setFechaCreacion(new Date());
                nuevoTrabajador.setUltimaFechaActualizacion(new Date());
                ...
                for (RnGcNomSolicitudesLineasTbl lineaOriginal : lineasOriginales) {
                    ...
                    nuevaLinea.setSolicitudTrabajadorId(nuevoTrabajador.getId());

                    // Copiar el tipo de línea
                    nuevaLinea.setTipoClave(lineaOriginal.getTipoClave());
                    nuevaLinea.setTipoConcepto(lineaOriginal.getTipoConcepto());
                    nuevaLinea.setTotalGravado(lineaOriginal.getTotalGravado());
                    nuevaLinea.setTotalExento(lineaOriginal.getTotalExento());
                    nuevaLinea.setDiasIncapacidad(lineaOriginal.getDiasIncapacidad());

                    // Percepción, deducción o incapacidad
                    nuevaLinea.setPercepcionId(lineaOriginal.getPercepcionId());
                    nuevaLinea.setDeduccionId(lineaOriginal.getDeduccionId());
                    nuevaLinea.setTipoIncapacidadId(lineaOriginal.getTipoIncapacidadId());

                    // Campos comunes
                    nuevaLinea.setCreadoPor(usuarioFirmado.obtenerIdUsuario());
                    nuevaLinea.setFechaCreacion(new Date());
                    nuevaLinea.setUltimaActualizacionPor(usuarioFirmado.obtenerIdUsuario());
                    nuevaLinea.setUltimaFechaActualizacion(new Date());

                    solicitudesLineasFacade.create(nuevaLinea);
                    System.out.println("   ➕ Línea duplicada para trabajador nuevo ID: " + nuevoTrabajador.getId());
                }

                System.out.println("   ➕ Trabajador duplicado [" + contador++ + "]: ID original: " + original.getId() + " → nuevo ID: " + nuevoTrabajador.getId());

            }

            // Mensaje final
            System.out.println("✅ Proceso finalizado. Nueva nómina creada con ID: " + nuevaNomina.getId());
            System.out.println("📝 Detalles:");
            System.out.println("   - Período: " + nuevoPeriodo.getNombrePeriodo());
            System.out.println("   - Solicitud ID: " + nuevaSolicitud.getId());
            System.out.println("   - Total trabajadores duplicados: " + nuevosTrabajadores.size());

        } catch (Exception e) {
            System.out.println("❌ Error al duplicar nómina: " + e.getMessage());
            e.printStackTrace();
        }
    }
