...
@Named("rnGcFacturasTblController")
@ViewScoped
public class RnGcFacturasTblController implements Serializable {

    @EJB
    private RnGcFacturaslineasTblFacade rnGcFactlineasTblFacade;

    @EJB
    private mx.com.rocketnegocios.beans.RnGcFacturasTblFacade ejbFacade;
    private List<RnGcFacturasTbl> items = null;
    private RnGcFacturasTbl selected;
    private UsuarioFirmado usuarioFirmado = new UsuarioFirmado();

    private RnGcFacturaslineasTblController faclineasController;

    public RnGcFacturasTblController() {
    }

    public RnGcFacturasTbl getSelected() {
        return selected;
    }

    public void setSelected(RnGcFacturasTbl selected) {
    ...
    private RnGcFacturasTblFacade getFacade() {
        return ejbFacade;
    }

    public RnGcFacturasTbl prepareCreate() {
        selected = new RnGcFacturasTbl();
        initializeEmbeddableKey();
        return selected;
    }

    public void create() {
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/Bundle").getString("RnGcFacturasTblCreated"));
    ...
    public void update() {
        persist(PersistAction.UPDATE, ResourceBundle.getBundle("/Bundle").getString("RnGcFacturasTblUpdated"));
    }

    public void destroy() {
        persist(PersistAction.DELETE, ResourceBundle.getBundle("/Bundle").getString("RnGcFacturasTblDeleted"));
        if (!JsfUtil.isValidationFailed()) {
    ...
    public List<RnGcFacturasTbl> getItems() {
        if (items == null) {
    ...
    private void persist(PersistAction persistAction, String successMessage) {
        if (selected != null) {
            ...
            System.out.println(selected.getId() + " ! "
                    + selected.getUuid() + " ! "
                    + selected.getRfcEmisor() + " ! "
                    + selected.getNombreEmisor() + " ! "
                    + selected.getRfcReceptor() + " ! "
                    + selected.getNombreReceptor() + " ! "
                    + selected.getLugarExpedicion() + " ! "
                    + selected.getFechaExpedicion() + " ! "
                    + selected.getTipoComprobante() + " ! "
                    + selected.getClaveRegimenFiscal() + " ! "
                    + selected.getUsoCfdi() + " ! "
                    + selected.getFormaPago() + " ! "
                    + selected.getMetodoPago() + " ! "
                    + selected.getMoneda() + " ! "
                    + selected.getImporte());
            try {
    ...
    public List<RnGcFacturasTbl> getItemsAvailableSelectMany() {
        return getFacade().findAll();
    }

    public List<RnGcFacturasTbl> getItemsAvailableSelectOne() {
        return getFacade().findAll();
    }

    RnGcTransaccionesTbl find(Integer id) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @FacesConverter(forClass = RnGcFacturasTbl.class)
    public static class RnGcFacturasTblControllerConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            RnGcFacturasTblController controller = (RnGcFacturasTblController) facesContext.getApplication().getELResolver().
                    getValue(facesContext.getELContext(), null, "rnGcFacturasTblController");
            return controller.getRnGcFacturasTbl(getKey(value));
        }
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof RnGcFacturasTbl) {
                RnGcFacturasTbl o = (RnGcFacturasTbl) object;
                return getStringKey(o.getId());
            } else {
                Logger.getLogger(this.getClass().getName()).log(Level.SEVERE, "object {0} is of type {1}; expected type: {2}", new Object[]{object, object.getClass().getName(), RnGcFacturasTbl.class.getName()});
                return null;
            }
        }
    ...
    public boolean filterByFecha(Object value, Object filter, Locale locale) {
        ...
        try {
            ...
            Logger.getLogger(RnGcFacturasTblController.class.getName()).log(Level.SEVERE, null, pe);
            return false;
        }
    ...
    public void leerXML() throws JDOMException, IOException, ParseException {
        prepareCreate();
        faclineasController = new RnGcFacturaslineasTblController();
        RnGcFacturaslineasTbl facturas = faclineasController.prepareCreate();
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (valor.equals("Conceptos")) {
                    for (int j = 0; j < conceptos.size(); j++) {
                        ...
                        facturas.setClaveProdServ(claveprodserv);
                        System.out.println("Cantidad: " + dato1.getAttributeValue("Cantidad"));
                        facturas.setCantidad(Float.valueOf(dato1.getAttributeValue("Cantidad")));
                        System.out.println("Clave Unidad: " + dato1.getAttributeValue("ClaveUnidad"));
                        facturas.setClaveUnidad(dato1.getAttributeValue("ClaveUnidad"));
                        System.out.println("Descripcion: " + dato1.getAttributeValue("Descripcion"));
                        facturas.setDescripcion(dato1.getAttributeValue("Descripcion"));
                        System.out.println("Valor Unitario: " + dato1.getAttributeValue("ValorUnitario"));
                        facturas.setValorUnit(Float.parseFloat(dato1.getAttributeValue("ValorUnitario")));
                        System.out.println("Importe: " + dato1.getAttributeValue("Importe"));
                        facturas.setImporte(Float.parseFloat(dato1.getAttributeValue("Importe")));
                        //Impuestos
                                ...
                                if (impues.equals("Traslados")) {
                                    for (int l = 0; l < traslado.size(); l++) {
                                        ...
                                        if (tras.equals("Traslado")) {
                                            ...
                                            facturas.setBase(Float.parseFloat(trasla.getAttributeValue("Base")));
                                            System.out.println("Impuesto: " + trasla.getAttributeValue("Impuesto"));
                                            facturas.setImpuesto(Integer.valueOf(trasla.getAttributeValue("Impuesto")));
                                            System.out.println("Tipo Factor: " + trasla.getAttributeValue("TipoFactor"));
                                            facturas.setTipoFactor(trasla.getAttributeValue("TipoFactor"));
                                            System.out.println("Tasa Cuota: " + trasla.getAttributeValue("TasaOCuota"));
                                            facturas.setTipoTasa(Float.parseFloat(trasla.getAttributeValue("TasaOCuota")));
